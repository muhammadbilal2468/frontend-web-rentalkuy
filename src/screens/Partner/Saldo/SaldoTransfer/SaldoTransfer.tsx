...
import "./SaldoTransfer.css";

const SaldoTransfer: React.FC = () => {
  const [viewSaldoTransfer, setViewSaldoTransfer] = useState<Photo>();
  ...
  const columns: TableColumn<Photo>[] = [
    ...
    },
    {
      ...
      render: (data) => {
        return (
          <div className="btn-saldo-ditransfer">
            <div className="btn-eye" onClick={() => setViewSaldoTransfer(data)}>
  ...
  return (
    <div className="saldo-transfer">
      ...
      <ViewBarang2
        data={viewSaldoTransfer}
        visible={viewSaldoTransfer != undefined}
        onClose={() => setViewSaldoTransfer(undefined)}
        status="Penyewa"
      />
    </div>
  );
